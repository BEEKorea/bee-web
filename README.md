...
1. Edit files locally using your preferred editor
   선호하는 편집기로 로컬에서 파일을 편집합니다

2. Test locally by opening `index.html` in your browser (optional)
   브라우저에서 `index.html`을 열어 로컬에서 테스트합니다 (선택사항)
...
4. Push to GitHub / GitHub에 푸시합니다:
   ```bash
...
1. Find the page in `pages/` or `en/pages/`
   `pages/` 또는 `en/pages/`에서 페이지를 찾습니다
...
1. Place images in the `images/` folder
   `images/` 폴더에 이미지를 넣습니다

2. Reference them in HTML: `/images/your-image.png`
   HTML에서 참조합니다: `/images/your-image.png`
...
1. Edit `nav.html` or `footer.html`
   `nav.html` 또는 `footer.html`을 편집합니다

2. Changes apply to all pages automatically
   변경사항이 모든 페이지에 자동으로 적용됩니다
...
**Language parity / 언어 동등성**: If you update Korean content, consider updating the English version in `en/`
한국어 콘텐츠를 업데이트하는 경우 `en/`의 영어 버전도 업데이트하는 것을 고려하세요

**LMS integration / LMS 통합**: The site includes links to `/lms/*` which connects to a separate Learning Management System
사이트에는 별도의 학습 관리 시스템에 연결되는 `/lms/*` 링크가 포함되어 있습니다

**Cache busting / 캐시 무효화**: The nav/footer includes version parameters to ensure fresh content loads
nav/footer에는 새로운 콘텐츠가 로드되도록 버전 매개변수가 포함되어 있습니다
